...
const SmartAudits = () => {
  ...
  return (
    <div className='bg-gradient-to-b from-black via-gray-800 to-black min-h-screen p-6 sm:p-10 lg:p-20'>
      <h1 className='text-4xl sm:text-5xl text-center text-white font-bold mb-12 sm:mb-16'>Latest Works</h1>
      <div className='flex flex-wrap justify-center items-center mb-4'>
        {Object.keys(categoryData).map((category) => (
          <button
            key={category}
            onClick={() => changeTab(category)}
            className={`mr-2 sm:mr-4 mb-2 sm:mb-0 px-3 py-1 rounded-full sm:rounded-2xl text-sm sm:text-md font-semibold focus:outline-none ${
        ...
        {categoryData[activeTab].map((project) => (
          <div key={project.id} className='bg-opacity-10 lg:space-x-12 mx-auto max-w-md sm:max-w-5xl rounded-xl bg-white bg-blur-md border-2 border-white space-y-4 sm:space-y-0 border-opacity-30 text-white px-6 sm:px-8 py-4 flex flex-col items-center sm:flex-row '>
            ...
            <div className='flex-1 sm:border-r-2 mx-auto max-w-4xl space-y-4 sm:border-white sm:border-opacity-10'>
              ...
              <p className='text-white text-sm lg:w-[600px] sm:text-base'>{project.description}</p>
            ...
            <div className='mt-4 sm:mt-0 flex justify-center items-center sm:justify-end'>
              <button className='bg-transparent border-2 border-white border-opacity-30 px-3 sm:px-4 hover:bg-white hover:text-black py-1 rounded-full sm:rounded-2xl text-sm sm:text-md font-semibold text-white'>
                View Audit
              </button>
            </div>
          </div>
